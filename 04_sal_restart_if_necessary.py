...
@author: sbe002
"""
#%% To check date in your _rst.nc file

## this is to check what restart file is good to use
last_coupling = 210
last_seedling_prod = 208

## After this check section of 
##%% check the restart setting
...
coupling_period_is = coupling_period*24*3600 
# time required in model to achieve same coupling period (in seconds)
coupling_period_model = coupling_period_is/MorFac 

#######################
## date equal to last coupling information
eq_to_timestep = last_coupling * coupling_period_model
eq_to_date = refdatet + datetime.timedelta(seconds=float(eq_to_timestep))
# print(eq_to_date)
## date equal to seedling production
eq_to_seed_ts = last_seedling_prod * coupling_period_model
eq_to_seed_date = refdatet + datetime.timedelta(seconds=float(eq_to_seed_ts))
# print(eq_to_seed_date)

diff = eq_to_seed_date - refdatet
diff_seconds = diff.total_seconds()

print('TStart is', diff_seconds)
print('Find _rst.nc for RestartFile with date of',eq_to_seed_date)
print('RestartDateTime is', eq_to_seed_date)
...
#%% Re Check the restart settings
start_run = model_dfm.get_start_time()
end_run = model_dfm.get_end_time()
delta_run = end_run - start_run
...
coupling_ntime =delta_run/coupling_period_model # how many coupling is 
...
## maka seedling final pos akan sama dengan last seedling coupling
# add 204, kemudian tambah 1 tahun di age dan add 208 tanpa tambah tahun
seed_1yr = os.path.join(MFON_OUT, 'Seedlings', 'Seedling_Coupling_'+str(last_seedling_prod-4) + '.txt')
seed_0yr = os.path.join(MFON_OUT, 'Seedlings', 'Seedling_Coupling_'+str(last_seedling_prod) + '.txt')

pd_seed_0yr = pd.read_csv(seed_0yr)
pd_seed_1yr = pd.read_csv(seed_1yr)
## add age 1yr to pd_seed_1yr
pd_seed_1yr['Age'] = pd_seed_1yr['Age']+datetime.timedelta(days = 365 )
seedling_finalpos = pd.concat([pd_seed_1yr, pd_seed_0yr])

# This is to modify the MesoFON parameters which should be in the current condition
# is at the position of last coupling iteration
Surv_Source = 'coupling'+str(last_coupling)
Sal_Source = Surv_Source
Excel_Source = Surv_Source
...
read_data = pd.read_csv(os.path.join(MFON_OUT_compile,'Coupling_'+str(last_seedling_prod)+'.txt'))
...
#%% Preparing the model
# check_curyr = eq_to_seed_ts*MorFac
# restart_delta_model = datetime.timedelta(seconds=check_curyr)
# curyr_check = ((refdatet+restart_delta_model).strftime('%Y'))
# print('Restart time is', ((refdatet+restart_delta_model).strftime('%Y%m%d %HH:%MM:%SS'))) 
...
print('Restart time in DFM model is', ((refdatet+cursec).strftime('%Y%m%d %HH:%MM:%SS'))) 
print('Restart time in coupling perspective is', ((refdatet+cursecMF).strftime('%Y%m%d %HH:%MM:%SS'))) 
...
# ntime = last_seedling_prod

#%% Run the Coupling

for ntime_restart in range(int(coupling_ntimeUse)):
    ntime = last_seedling_prod+ntime_restart
